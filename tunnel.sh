...
#add BIG-IP 10.0.3.0 vtep ip 10.169.72.34 vni 68 vtep mac 00:50:56:a0:7d:d8
    kubectl exec -it $CA -n kube-system -- bpftool map update id $tunnelid key hex 0a 00 03 00 00 00 00 00  00 00 00 00 00 00 00 00 01 00 00 00 value hex 0a a9 48 22 44 00 00 00  00 50 56 a0 7d d8 00 00 01 00 00 00

    kubectl exec -it $CA -n kube-system -- cilium bpf tunnel list

#    vxlanmac=$(kubectl exec -it $CA -n kube-system -- ip link list cilium_vxlan | tail -1 | awk '{print $2;}')
